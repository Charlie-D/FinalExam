...
public class Retirement {
	...
	public double MonthlySavings() {
		...
		double pmv = Math.abs(Math.round(PMT(this.dAnnualReturnWorking / 12, this.iYearsToWork * 12, 0, TotalAmountToSave(), false)
				 * 100.0)) / 100.0;
	...
	public double TotalAmountToSave() {
		...
		double pv = Math.abs(Math.round(PV(this.dAnnualReturnRetired / 12, this.iYearsRetired * 12, this.dRequiredIncome - this.dMonthlySSI, 0, false)
				 * 100.0)) / 100.0;
		return pv;
	}
